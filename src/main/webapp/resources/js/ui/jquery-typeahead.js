 ...
 */
(function (factory, style) {
    if (typeof define === "function" && define.amd) {
        define(["ui/jquery", "ui/utils"], function (jQuery, uiUtils) {
        	uiUtils.setDfeCustomStyle(style);
    ...
    window.Typeahead = new Map();
    window.Typeahead.version = '2.10.4';
    ...
    Typeahead.prototype = {
        ...
        delegateEvents: function () {
            ...
            $("html")
                .off("touchmove touchstart")
                .on("touchmove", function () {
                    Array.from(window.Typeahead.values()).forEach(function(scope){
     ...
     */
    $.fn.typeahead = $.typeahead = function (options, command) {
        return _api.typeahead(this, options, command);
    };
    ...
    var _api = {
         ...
         * @returns {*}
         */
        typeahead: function (node, options, command) {
            var target = window.Typeahead.get(node[0]);
            command = (command || options);
            if( target && typeof command === 'string') {
                if( command === 'reload') {
                    target.resetInput();
                    target.options.source = options.source;
                    target.unifySourceFormat();
                    target.generateSource(Object.keys(target.options.source));
                }
                if( command === 'clean') 
                    target.resetInput();
                if( command === 'destroy') 
                    window.Typeahead['delete'](target);
                if( command === 'close') 
                    target.hideLayout();
                if( command === 'showloading' )
                    target.options.loadingAnimation && target.container.addClass("loading");
                if( command === 'hideloading' )
                    target.options.loadingAnimation && target.container.removeClass("loading");
                return ;
            }
            ...
            if (node.length === 1) {
                ...
                window.Typeahead.set(node[0], new Typeahead(node, options));
            } else {
                /*var instances = {},
                    instanceName;
    ...
    _debug.log({ message: "WARNING - You are using the DEBUG version. Use /dist/jquery.typeahead.min.js in production." });

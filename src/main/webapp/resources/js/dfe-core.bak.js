define('dfe-core', function() {
    ...
    function createElement(type, attributes, children) {
        ...
        if(attributes instanceof Node) {
            let info = attributes.getImmediateNodeInfo();
    ...
    class DOM {
        static calcImmediateNodeInfo(renderStructure, fpos) {
            let fi = 0; 
            return renderStructure.reduce( (pos, st) => pos.concat( st instanceof Node ? st.getImmediateNodeInfo() : fpos[fi++]||{} ) , [] );
        }
        ...
        static batchRender(nodes) {
            nodes.forEach(node => DOM.render(node));
        }
        
        static render(node) {
            if( node.isAttached() ) {
                node.getImmediateNodeInfo();
                if( node.renderStructure !== node.lastRenderStructure && node.parentDOMElements != 0 ) {
                    node.children.forEach( map => map.forEach( child => child.attached = false ) );
                    ...
                    /*node.children.forEach(
                        fieldMap => fieldMap.forEach(
                            child => child.attached || child.parentDOMElements == 0 || DOM.applyDOMChanges(child, child.parentDOMElements = [], child.lastRenderStructure, child.lastRenderStructure )
                        )
                    )*/
        ...
        static calcRenderStructure(node) {
            ...
            if( node.parent && !node.parent.shouldRecalculateRenderStructure ) { // && node.parent.attachedChildNodes.has(node) ) {
    ...
    class Node {
        constructor(parent, field, unboundModel, runtime) {
            Object.assign(this, {
                ...
                shouldRecalculateRenderStructure: false, // until  'logic' returns something ...
                parentDOMElements: [], 
        ...
        getImmediateNodeInfo() {
            if(this.shouldRecalculateRenderStructure) {
                DOM.calcRenderStructure(this)
                this.shouldRecalculateRenderStructure = false;
            }
            return this.immediateNodeInfo;
        }
        isAttached() {
            let node = this;
            for(; node && node.attached; node = node.parent) ;
            return !node;
        }
        notify(action) {

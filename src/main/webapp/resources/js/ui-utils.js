...
// strictly IE 7/8. rest use es5/es6 shims
...
var ajaxCache = (function() {
    /*function _Promise(f){
        var t = this;
        t.waitRs = [], t.waitRj = [];
        f(function(r){ t.done = 1; t.doneRs = 1; t.dataRs = r; t.waitRs.forEach(function(f) { f(r) }); t.waitRs = []; t.waitRj = []; },
          function(r){ t.done = 1; t.doneRj = 1; t.dataRj = r; t.waitRj.forEach(function(f) { f(r) }); t.waitRj = []; t.waitRs = []; })
        t.then = function(rs, rj){ 
            this.doneRj ? rj && rj(this.dataRj) : this.doneRs ? rs && rs(this.dataRs) : (rs && this.waitRs.push(rs), rj && this.waitRj.push(rj));
            return this;
        }
        t.fail = function(rj){ return this.then(void 0, rj) }
    }*/
    ...
    return {
                ...
                storage.set(key, v = new Promise(function(resolve, reject){
                    ...
                    xhr.onreadystatechange = function() {
                        if(this.readyState === 4) {
                            ...
                            if(this.status == 200)
                                try {
                                    resolve(typeof this.response == 'string' && dataType == 'json' ? JSON.parse(this.response) : this.response)
                                } catch(e) {
        ...
        put: function(key, response, dataType) {
            var r = 0, d, v;
            storage.set(key, v = {
                done: 1,
                then: function(resolve, reject) {
                    if(!r) {
                        try {
                            d || (d = typeof this.response == 'string' && dataType == 'json' ? JSON.parse(this.response) : this.response);
                            resolve(d)
                        } catch(e) {
                            r = {xhr: { response: response, responseText: JSON.stringify(response), status: 200, statusText: 'OK' }, exception: e}
                        }
                    }
                    r && reject(r);
                    return v;

...
var JavaCacheHandler = Java.type('com.arrow.util.experimental.CacheHandler');
			...
			return new Promise(function(resolve, reject) {
				var task = EventLoop.createQueueTask(function(deps, e){
					if( !(e instanceof JavaException || e instanceof Error) ) {
						try {
							typeof cb == 'function' ? resolve(cb.apply(null, deps)) : resolve(deps);
							return ;
						} catch (ex) {
							e = ex;
						}
					...
					reject(e);
					typeof rj == 'function' ? rj(e) : console.warn('Unhandled error while requiring modules', d, e);
				}, 'require');
...
var validateDfe = (function(){
	...
	return function (jsonModel, formClass, timeLimit) {
		...
		var errors = [], runtime = new Core.Runtime(listener()).setDfeForm(formClass).setModel(JSON.parse(jsonModel)), failure;
		EventLoop.run(function() {
			runtime.schedule.push(function(){ runtime.stop() });
			try {
			    runtime.restart(null, 'validate');
			} catch(e) {
                failure = e;
			}
		}, timeLimit*1000);
